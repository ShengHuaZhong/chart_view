...
int AppGuard::argc = 1;
char *AppGuard::argv[] = {const_cast<char *>("s101_symbolized_smoke_tests"), nullptr};

FeatureChartDataset makeSmokeDataset()
{
  FeatureChartDataset dataset;
  DatasetMeta meta;
  meta.name = "s101_symbolized_smoke";
...
TEST_CASE("S101 symbolized smoke renders semantic styles, priority, and labels through SENC", "[s101][symbolized][smoke][rhi]")
{
  AppGuard guard;
  ...
  renderer.portrayalRegistry().registerTextRuleForStyle(
    "text/default",
    chart_view::runtime::portrayal::TextRule{{28U, 180U, 62U, 255U}, 12U});
  ...
  REQUIRE(frameHasColor(rgba, {188U, 178U, 112U, 255U}));
  REQUIRE(frameHasColor(rgba, {18U, 122U, 150U, 255U}));
  REQUIRE(frameHasColor(rgba, {176U, 68U, 22U, 255U}));
  REQUIRE(regionHasColor(rgba, 800, 404, 294, 430, 310, {28U, 180U, 62U, 255U}));

  const auto centerOffset = static_cast<std::size_t>((300 * 800 + 400) * 4);
  REQUIRE(centerOffset + 3 < rgba.size());
  REQUIRE(rgba[centerOffset + 0] == 176U);
  REQUIRE(rgba[centerOffset + 1] == 68U);
  REQUIRE(rgba[centerOffset + 2] == 22U);
